...
> **This fork** (`@ai-hero/sandcastle`) ships with a [Z.ai](https://z.ai) provider extension for the Pi agent, making Z.ai models the default. See [Z.ai model IDs](#zai-model-ids) for important details on model ID resolution.
...
3. Edit `.sandcastle/.env` and fill in your default values for `ZAI_API_KEY` (Z.ai models) or `ANTHROPIC_API_KEY` (Claude models). If you want to use your Claude subscription instead of an API key, see [#191](https://github.com/mattpocock/sandcastle/issues/191).
...
This fork bundles a Z.ai provider extension for the Pi agent. Pi's built-in `opencode` provider already registers models with bare IDs like `glm-5.1`. The Z.ai extension registers the same models under the `zai/` prefix (e.g. `zai/glm-5.1`).

**Always use the fully-qualified `zai/` prefix** when specifying Z.ai models to avoid collisions with the built-in provider:
...
await run({
  agent: pi("zai/glm-5.1"), // ✅ correct — uses Z.ai provider
  // agent: pi("glm-5.1"),    // ❌ ambiguous — may resolve to opencode's model
  sandbox: docker(),
  prompt: "Fix issue #42",
});
```

Available Z.ai models: `zai/glm-5.1`, `zai/glm-5-turbo`, `zai/glm-4.7`, `zai/glm-4.5-air`.

The `sandcastle init` scaffold outputs `zai/`-prefixed model IDs automatically. Templates generated by init are already correct — you only need to remember the prefix when constructing `pi()` calls by hand.
...
| Option               | Type               | Default                       | Description                                                                                                                                                     |
| -------------------- | ------------------ | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent`              | AgentProvider      | —                             | **Required.** Agent provider (e.g. `claudeCode("claude-opus-4-6")`, `pi("zai/glm-5.1")`, `codex("gpt-5.4-mini")`, `opencode("opencode/big-pickle")`)            |
